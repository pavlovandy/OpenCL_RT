...
/*   Updated: 2019/09/26 19:01:45 by ozhyhadl         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	make_little_default_scene(t_scene *scene)
{
	...
	// scene->obj[1].shape.cylin.dot = (cl_double3){{0, 0, 0}};
	// scene->obj[1].shape.cylin.dir = ft_normalize((cl_double3){{1, 0, 0}});
	...
	scene->obj[1].shape.cone.dir = ft_normalize((cl_double3){{0, 0, 1}});
	scene->obj[1].shape.cone.tangent = (cl_double)0.3;
	scene->obj[1].shape.cone.mmin = 0;
	scene->obj[1].shape.cone.mmax = 5;
	scene->obj[1].rotation = (cl_double3){{1 * M_PI / 180.0, 0 * M_PI / 180.0, 0 * M_PI / 180.0}};
	scene->obj[1].rotation_martix = build_rotation_matrix_for_dir(scene->obj[1].shape.cone.dir);
	...
	scene->obj[2].shape.disk.normal = ft_normalize((cl_double3){{1, 1, 1}});

	scene->obj[2].rotation = (cl_double3){{0, 0, 0}};
	scene->obj[2].rotation_martix = build_rotation_matrix_for_dir(scene->obj[2].shape.disk.normal);
