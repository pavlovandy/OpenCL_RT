...
/*   Updated: 2019/09/28 12:21:22 by apavlov          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int			get_object_intersection(t_rt *rt, int x, int y)
{
	...
	ret = clEnqueueTask(rt->cl.command_queue, \
		rt->cl.click_kernel, 0, NULL, NULL);
	if (ret != CL_SUCCESS)
		exit(error_message(RED"Oops"COLOR_OFF));
	ret = clEnqueueReadBuffer(rt->cl.command_queue, \
	rt->cl.id_obj, CL_FALSE, 0, sizeof(cl_int), &id_obj, 0, 0, 0);
	if (ret != CL_SUCCESS)
		exit(error_message(RED"Oops"COLOR_OFF));
